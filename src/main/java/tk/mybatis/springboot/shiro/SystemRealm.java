...
public class SystemRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		 ...
		 * List<Role> roles = userService.getRoles(current.getId()); List<String>
		 * roleList = new ArrayList<String>(); for (Role role : roles) {
		 * roleList.add(role.getName()); }
		 * 
		 * SimpleAuthorizationInfo simpleAuthorInfo = new SimpleAuthorizationInfo(); if
		 * (current.getName().equals("syj")) { simpleAuthorInfo.addRoles(roleList);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		UserInfo user = userService.getByUsername(token.getUsername());
